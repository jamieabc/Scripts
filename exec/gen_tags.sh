...
if [ "$1" == "e" ]; then
    ...
    if [ ! -f GTAGS ]; then
        ...
        find . -type f -name "*.js" -o -name "*.jsx" -o -name "*.html" -o -name "*.json" -o -name "*.rb" -o -name "*.c" -o -name "*.h" -o -name "*.cpp" | grep -vE '(node_modules|buld|.min.js$|__site_|.git)' > gtags.files
        gtags -f gtags.files
    else
