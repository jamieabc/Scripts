...
brew install bazaar readline git wget imagemagick coreutils findutils gnu-tar gnu-sed gawk gnutls gnu-indent gnu-getopt autojump tig htop tree w3m ascii gcc bash-completion ack gpg gpg2 the_silver_searcher ispell cmake surfraw dos2unix doxygen fasd python python3 pstree calcurse ranger tmux reattach-to-user-namespace mongodb terminal-notifier go socat mytop most pandoc fcitx-remote-for-osx grc fish mtr ical-buddy tidy-html5 axel jq ssh-copy-id ansible ffmpeg kafka protobuf mono gnu-tar tldr editorconfig flow httpie mycli go latex2html watchman unrar ripgrep chezscheme percol nvm homebrew/x11/zentify sbcl redis libucl argon2 kubernetes-cli postgresql awscli gnupg gnupg2 carthage gpg-agent graphviz git-quick-stats cloc bat prettyping fzf diff-so-fancy fd autoconf automake gettext libtool noti ponysay aws-shell dep vegeta cask pgcli wdiff colordiff sonarqube sonar-scanner shellcheck berkeley-db4 boost miniupnpc openssl pkg-config qt libevent qrencode dnsmasq hardlink-osx calc ccrypt links lynx irssi mosh p7zip lnav libuv libmicrohttpd hub ghi ansible watch berkeley-db libnet libnids libpcap openssl mc leveldb highlight libxml2 librsvg cairo texinfo sha3sum exercism tmuxinator zsh-completions hub fd stunnel
